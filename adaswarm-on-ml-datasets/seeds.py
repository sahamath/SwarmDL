...
#Load Seeds
seeds = load_seeds()
X = seeds[0].astype(float)
y = seeds[1].astype(float)
y = to_categorical(y)
...
optimizer_test = 'SGD'
model = Model(n_features=7, n_neurons=2, n_out=3)
...
optimizer_test = 'DiffGrad'
model = Model(n_features=7, n_neurons=2, n_out=3)
optimizer = torch.optim.DiffGrad(params=model.parameters())
...
for e in range(num_epochs):
    ...
    for ix, (_x, _y) in enumerate(ds):
        ...
        batch_losses.append(loss.item())
        print("Batch : {}| Loss: {} | Time: {}".format(ix, loss.item(), toc-tic))
    diffgrad_losses.append(sum(batch_losses) / num_batches)
    diffgrad_accuracy.append(100 * sum(accs) / num_batches)
    if e % 1 == 0:
        print("[{}/{}], loss: {} acc: {}".format(e,
                                                 num_epochs, np.round(sum(batch_losses) / num_batches, 3),
                                                 100 * np.round(sum(accs) / num_batches, 3)))

print(diffgrad_losses)
print(diffgrad_accuracy)
